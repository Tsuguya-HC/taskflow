...
短縮名（`kubectl get task`）が曖昧になるかどうかは、**そのクラスタに他に何が入っているかで決まる**
（`Task` を名乗る kind は他にもある）ので、group の選択で保証できるのは完全修飾名の側だけ。
...
設定ミスがコントローラのエラーではなく、**通信が黙って消える**形で出る。
...
→ **答えは `runner: State`（[ADR-0011](adr/0011-verdict-from-declared-state.md)）で、
足す機構は無い。** 攻撃者が書けるテキストを読むフェーズと終端の間に、framework が起こさない run を
1 つ置く。そこに現れる値は誘導されたエージェントが書いたものではなく、**その置き場を書ける権限が
書いたもの**になる。

**framework は「そこを通れ」を強制しない。** 強制する案（フェーズに `untrusted` / `gate` の印を
足し、`untrusted` な始点から成功終端へ至る全経路が `gate` を通ることを admission で検査する）は
採らない。理由は 3 つで、どれも ADR-0011 の帰結:

- **通ったことは承認されたことではない。** コントローラは誰が値を書いたかを知り得ない
  （P7、ADR-0011 決定1 が `External` を落とした理由そのもの）。検査が保証できるのは
  「経路上に `State` のフェーズが 1 つ宣言されている」までで、そこを埋めたのが誘導された
  エージェント自身でも通る。**実際に守るのは経路ではなく、その置き場を書ける権限**（決定4）
- **印が自己申告になる。** 検査が発火するのは `untrusted` を付けた flow だけで、付け忘れた flow は
  検査対象ゼロのまま通る。印を付けるのは flow を書く側なので、守りたい相手と申告する側が同じになる
- **提供側の物差しに掛からない。** この検査が無くてもコントローラの配管は定義できる（§2）。
  「定義はできるが結果が悪いだけ」はポリシー層で、P2 の側
...
**public にできる。** コントローラが知っているのは core API と自分の CRD だけで、クラスタ固有の名前も
接続先も秘密も持たない（`storageClassName` を既定に含めないのも同じ理由 — [ADR-0002](adr/0002-per-task-workspace-pvc.md) 決定3）。
守るべき秘密はすべて利用側のリポジトリに残るので、この分割は可視性の分割でもある。
