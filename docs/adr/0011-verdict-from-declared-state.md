...
3. **watch しない。待っている run があるときだけ uncached の `Get` を引き、requeue で再訪する。**
   RBAC は `configmaps` の **`get` だけ**で済み、`list` / `watch` が要らない = informer の
   キャッシュにクラスタ中の ConfigMap が乗らない。**どの ConfigMap を verdict 源にしてよいかを
...
4. **`State` では `timeout` を必須にする**（CEL で create 時に拒否）。期限の無い待ちは沈黙と
...
5. **棚は次の run の prepare が `results/<runID>/<value>/` を空で敷く。**
...
- **待ち中の再確認間隔の既定**。`timeout` とは別に、requeue の間隔を決める必要がある。
  ...
  遅延はここで決まる。実測は #112
- **先行調査が未実施**。同じ決定（外の判断を待つフェーズ）を切った先行者は実在する
  （suspend / resume を持つワークフローエンジン、承認を専用 kind で表す実装）。
  「2 本目の経路をどの層に置いたか」だけは実装前に一次資料で当たる
