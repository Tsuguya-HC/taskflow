...
- ~~`failurePolicy` を `Fail` にするか~~ → **`Fail`**（実装時、#17）。`Ignore` を採らないのは、
  ...
  **残る窓は 2 つ**: cert を注入する側が `caBundle` を埋めるまでの間と、
  drain と rollout が重なった場合。どちらも配置側が意図的に起こして測り（#107）、
  **窓は実在するが配置側の構成で塞がっている**ことが確かめられた。証明書の注入が止まった状態を
  作れば書き込みは実際に拒否される（TLS 検証に失敗するので webhook のタイムアウトを待たずに
  即座に失敗する）が、注入が戻れば GitOps の再試行がそのまま巻き戻す。replica を落としながら
  rollout を重ねても、拒否は 1 度も出なかった。**数字と手順は配置側の記録にある** —
  この ADR が持つのは「可用性は配置側の持ち物」という線引きだけで、
  どれだけ塞げているかはあちらが答える
