...
8. **実装の決め事**（2026-09-25 追記。実装を詰める中で決めた、上の決定を形にするための約束）
   - **枝が走っている間、`status.phase` は分岐元のフェーズのまま**。`currentRuns` に枝の run が並ぶ。この
     間だけ「走っている run のフェーズ = `status.phase`」が成り立たない。フェーズごとの回数（ADR-0012）は
     `status.phase` ではなく `currentRuns` から数える — 分岐元を二重に数えないため
   - **分岐元の答えは、書かれたディレクトリをソートして `/` でつないだ 1 つの文字列**。publish は `--many` で
     それを termination message の 1 行目に書き、コントローラは `/` で分けて各片が宣言されたディレクトリで
     重複が無いことだけを見る（`/` はディレクトリ名に使えないので分け方は 1 通り。パーサは増えない）。
     history の分岐元の行の `directory` も同じ文字列
   - **終端を決めた run の行を、同時に決着させる一群の最後に積む**。決着した他の枝 → `Cancelled` の枝 →
     決めた枝、の順。こうすると「Finally 以外で最後の history の行」が、直列でも並列でも終端を決めた run
     になり、finally の単一値 env と `inputs` はその行を読めば足りる
   - **`prev-run-id` annotation は「この run に至った run」**。枝なら分岐元の run、合流先なら最後に決着した枝
